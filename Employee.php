...
Find the account ID that made the most expensive purchase and have purchased more than 1 item. 
<input type="submit" name="search"  value="Go!"/>
    ...
    if($db_conn) {
    	if(isset($_POST['modify'])){
    		...
    		if(preg_match("/[0-9]+/",$_POST['EmployeeID'])){
    			...
    			if(!(strcmp($ename, "New Name")==0)){
    			if(preg_match("/[A-Z | a-z]+/",$_POST['EName'])){
...
<?php
		...
		$result_me = OCIExecute($parse_me, OCI_DEFAULT);
		OCICommit($db_conn);
	    $most_expensive2 ="	select m.Account_ID, n.purchase_total
				from NOS n, makes m
				where n.Sale_Number = m.Sale_Number and n.purchase_total = (select max(purchase_total) from nos)";	
		
	    $result_me2 = executePlainSQL($most_expensive2);
	
	echo "<br>Retrieved Data: <br>";
	echo "<table>";
	echo "<tr><th>Account ID</th><th>Purchase Total</th></tr>";
	while ($row = OCI_Fetch_Array($result_me2,OCI_BOTH)) {
		echo "<tr><td>" . $row["ACCOUNT_ID"] . "</td><td>" . $row["PURCHASE_TOTAL"] . "</td></tr>";
	  }
	  echo "</table>";
